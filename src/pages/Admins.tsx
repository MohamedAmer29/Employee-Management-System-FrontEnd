...
import { useAdmins } from "@/features/admins/admins.hooks";
import { useDepartments } from "@/features/employees/employees.hooks";
import { getAssetUrl } from "@/utils/assetUrl";
import { formatDateInUserZone } from "@/utils/formatDate";
import type { AdminsParams } from "@/api/user.api";
...
const AdminsPage = () => {
  ...
  const params = useMemo<AdminsParams>(
    () => ({
      page,
      limit,
                        ...
                        {admin.employee?.department ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-600 dark:text-gray-300">
                              {admin.employee.department.name}
                            </span>
                          </div>
                        ) : (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-500 dark:text-gray-400">
                            {admin.isEmailVerified
                              ? formatDate(admin.emailVerifiedAt)
                              : "Not verified"}
                          </span>
                        </div>
                      </td>
            ...
            <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {Math.min(startIndex + limit - 1, total)} of {total} admin
                  {total === 1 ? "" : "s"}
                </span>
                <select
                  value={limit}
