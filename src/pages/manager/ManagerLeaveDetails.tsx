...
const ManagerLeaveDetails = () => {
  const { employeeId } = useParams<{ employeeId: string }>();
  const navigate = useNavigate();
  const approveLeave = useManagerApproveLeave();
  const rejectLeave = useManagerRejectLeave();
  const {
    data: request,
    isLoading,
    isError,
    refetch,
  } = useManagerLeaveById(employeeId);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Leave details · {request.employee.email}
            </p>
          </div>
        </div>
        <button
...
export default ManagerLeaveDetails;
