...
import { usePayrollManager } from "@/features/payroll/payroll.hooks";
...
const AdminPayrollManager = () => {
  ...
  const { data: records = [], isLoading, isError, refetch } = usePayrollManager(
    managerId ?? "",
    params,
    !!managerId,
  );
  ...
  const manager = records[0]?.manager;
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-gray-50 tracking-tight">
            {manager?.fullName ?? "Manager"} — Payroll
          </h1>
          {manager && (
